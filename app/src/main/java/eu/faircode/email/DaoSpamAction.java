...
/** Low-level persistence used only by durable Spam Control undo/reset. */
@Dao
public interface DaoSpamAction {
    @Insert
    long insertAction(EntitySpamAction action);

    @Query("UPDATE spam_action SET state = :state, undone_at = :undoneAt WHERE id = :id")
    int setActionState(long id, int state, Long undoneAt);

    @Query("DELETE FROM spam_action WHERE id = :id")
    int deleteAction(long id);

    @Query("SELECT * FROM spam_action" +
            " WHERE state IN (" + EntitySpamAction.STATE_PENDING + "," + EntitySpamAction.STATE_APPLIED + ")" +
            " ORDER BY id DESC LIMIT 1")
    EntitySpamAction getLatestUndoable();

    @Query("SELECT * FROM spam_action" +
            " WHERE state IN (" + EntitySpamAction.STATE_PENDING + "," + EntitySpamAction.STATE_APPLIED + ")" +
            " ORDER BY id DESC LIMIT 1")
    LiveData<EntitySpamAction> liveLatestUndoable();
    ...
    @Query("SELECT * FROM spam_family ORDER BY id")
    ...
    @Query("SELECT * FROM spam_family_exemplar ORDER BY id")
    ...
    @Query("SELECT * FROM alias_delivery WHERE account_uuid = :accountUuid AND message_id = :messageId LIMIT 1")
